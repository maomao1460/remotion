...
const perspective = z.object({label: z.string().min(1), text: z.string().min(1)});
...
export const newsBriefInputSchema = z.object({
  ...
  captions: z.array(z.object({text: z.string().min(1).max(48), startMs: z.number().min(0), endMs: z.number().positive(), timestampMs: z.number().nullable(), confidence: z.number().nullable(), pageBreakAfter: z.boolean().optional()})).min(6).max(12),
  review: z.object({verification: z.literal('cross-checked'), approvalStatus: z.literal('approved'), sources: z.array(source).min(8).max(16), numericClaims: z.array(numericClaim).min(6).max(12)}),
